...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        if not contexts:
            return 0.0
        # Step 1: mark each chunk as relevant or not
        ...
        return min(1.0, max(0.0, float(ap_sum / num_relevant)))
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            f"You are an expert evaluator. Score the following AI answer.\n\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n\n"
        ...
        # 3. Parse scores from response -- find first {...} block
        ...
        # Fallback: default 0.5 per criterion if parse failed
        if not scores:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        # Collect every individual score value from all items in the batch
        ...
        # leniency / severity -- straightforward threshold on overall average
        leniency_bias = overall_avg > 0.8
        severity_bias = overall_avg < 0.3

        # positional_bias -- first item's avg score consistently higher than rest
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Pass retrieved_contexts only when non-empty so retrieval metrics
            # are computed; None -> context_recall/precision stay None.
            ...
            # Preserve original QAPair (including metadata / retrieved_contexts)
            result.qa_pair = pair
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        def _avg(attr: str) -> float:
            return sum(getattr(r, attr) for r in results) / total
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def _avg(res_list: list, attr: str) -> float:
            if not res_list:
                return 0.0
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # All three tied at the minimum -> multiple issues
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures):
            ...
            fix = suggestions[idx] if idx < len(suggestions) else (suggestions[0] if suggestions else "Investigate issue")
    ...
    def generate_improvement_suggestions(self, failures: list[EvalResult]) -> list[str]:
        ...
        # Priority order: most impactful failure types first
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append(
                "Refine system prompt and query routing to improve answer relevance"
            )
        if categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Add intent detection layer to redirect off-topic queries"
            )

        # Guarantee at least 3 suggestions regardless of failure distribution
        defaults = [
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement cross-encoder reranker to improve Context Precision",
            "Increase retrieval top-K and apply MMR to reduce noise chunks",
        ]
        for s in defaults:
